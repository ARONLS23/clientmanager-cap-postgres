...
class DocumentHandler {
    ...
    async normalizeBinaryContent(content) {
    ...
    async upload(req, serviceEntities) {
        ...
        const { Documents } = serviceEntities;
        ...
        const documentContent = await this.normalizeBinaryContent(content);
        ...
        try {
            await alertNotification.sendDocumentUploadedAlert(savedDocument, project);
        } catch (error) {
    ...
    async getContent(req) {
        ...
        const contentBuffer = await this.normalizeBinaryContent(document.content);
